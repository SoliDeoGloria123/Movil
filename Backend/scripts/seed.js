...
const createSampleCategories = async (adminUser) => {
  try {    const existingCategories = await Category.find();
    
    if (existingCategories.length > 0) {
      console.log('Limpiando categorías existentes para evitar conflictos...');
      await Category.deleteMany({});
      await Subcategory.deleteMany({}); // También limpiar subcategorías
      await Product.deleteMany({}); // También limpiar productos
    }const categories = [
      {
        name: 'Electrónicos',
...
const seedDatabase = async () => {
  try {
    ...
    // Crear categorías
    console.log('\n📁 Creando categorías...');
    const categories = await createSampleCategories(adminUser);    // Crear subcategorías
